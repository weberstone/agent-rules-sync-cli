...
describe('generateClaudeMd', () => {
  ...
  it('includes multiple framework rows for fullstack', () => {
    ...
    const fwLines = lines.filter(
      (l: string) =>
        l.includes('angular-guidelines.md') || l.includes('only-node.md'),
    );
...
describe('getGenerator', () => {
  it('returns correct generator for each agent', () => {
    expect(getGenerator('claude-code')).toBe(generateClaudeMd);
    expect(getGenerator('cursor')).toBe(generateCursorRules);
    expect(getGenerator('gemini-cli')).toBe(generateGeminiMd);
    expect(getGenerator('codex')).toBe(generateAgentsMd);
    expect(getGenerator('continue')).toBe(generateContinueRules);
  });
