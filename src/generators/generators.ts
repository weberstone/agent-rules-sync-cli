...
function generateClaudeMd(ctx: GeneratorContext): AgentFile[] {
  ...
  const body = rows
    .map(
      (r) =>
        `| ${r.priority} | \`@.agents/rules/${r.file}\` | ${r.description} |`,
    )
    .join('\n');
...
const REGISTRY: ReadonlyMap<AgentKey | 'gemini', AgentGenerator> = new Map([
  ['claude-code', generateClaudeMd],
  ['cursor', generateCursorRules],
  ['gemini-cli', generateGeminiMd],
  ['gemini', () => []], // Gemini web — no project-level config file to generate
  ['codex', generateAgentsMd],
  ['continue', generateContinueRules],
]);

export function getGenerator(key: string): AgentGenerator | undefined {
  return REGISTRY.get(key as AgentKey);
}
