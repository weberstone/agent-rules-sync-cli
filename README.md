...
* **1. Fork the project.** Create a fork to build a custom set of rules tailored to your personal projects or your team's specific needs.
* **2. Populate the library.** Fill the library with rules and guidelines based on the predefined architecture to match your or your team's requirements.
* **3. Host it in your GitHub repository.** Push the project or create a pull request to your private repository so your entire team has access to a centralized storage. If you are using the tool for personal purposes, you can just run the script locally.
...
It all depends on the tool making the edits; it can either append new data or overwrite everything from scratch. For example, at the time of writing, Claude Code overwrites the entire contents of the `CLAUDE.md` file during the `/init` process.
Therefore, we highly recommend running all automatic generations first, and only then synchronizing your skills and rules via `agent-context-sync-cli`.
The `agent-context-sync-cli` utility always appends to the file and never overwrites it completely. It uses isolated Safe Zones technology via special `<!-- BEGIN/END AGENT-RULES-SYNC -->` comments and only updates its manifest table. Any custom rules you added manually in `CLAUDE.md`, `.cursorrules`, or generated by other plugins will not be overwritten.
