...
- **Write Tests First**: Since we already discussed the expected behavior, write the tests (using **Vitest**) first.
...
- Run the Linter and Prettier (e.g., `npm run lint` / `npm run prettier:write`).
- Run the application build (`npm run build`) and type check (`npm run typecheck`) to ensure no compilation errors are introduced.
- Run tests (`npm test` / `ng test`) if applicable to the modified logic.
...
- Use this file as your "scratchpad". Document the current stage, chosen approaches, pending bugs, and immediate next steps.
